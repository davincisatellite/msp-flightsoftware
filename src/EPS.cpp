...
void fill_conf_variable(union ReturnType &par_value, uint8_t *buf, uint8_t n) {
...
void read_and_fill_conf_param(DWire &wire, uint8_t i2c_address, ConfigParameter par_id, EPS::config_reply &reply) {
    ...
    if (response == read_length) {
        ...
        fill_conf_variable(reply.par_value, buffer, param_length);
...
void print_overcurrent_reply(EPS::overcurrent_reply reply) {
    ...
    for(int i=0;i<8;i++) {
      	Console::log("    OCF_CNT_CH%d: %d | OCF_CNT_CH%d: %d", i*2, reply.ocf_cnt_ch[i*2], i*2+1, reply.ocf_cnt_ch[i*2+1]);
    }
}
