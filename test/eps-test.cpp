...
void print_uint8_array_bytes(const uint8_t* data, uint16_t len)
{
    if (data == nullptr) {
        Console::log("print_uint8_array_bytes: null pointer");
        return;
    }

    Console::log("Array dump (%d bytes):", (int)len);

    for (uint16_t i = 0; i < len; i++) {
        // Print index, hex, and decimal
        Console::log("  [%03d] = 0x%x (%d)",
                     (int)i,
                     (unsigned int)data[i],
                     (int)data[i]);
    }
